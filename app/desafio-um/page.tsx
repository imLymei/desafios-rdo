...
function Page() {
	...
	return (
		<div className='flex flex-col items-center gap-12 p-8'>
			<h1 className='text-4xl'>Conversor de números romanos</h1>

			<form onSubmit={handleCalc} className='relative flex flex-col gap-4 p-4 border border-white rounded-sm'>
				<div>
					<p>Número</p>
					<input required name='number' className='outline-none text-black p-1 rounded-sm' />
				</div>
				...
				<p className='absolute top-2 right-2 flex justify-center items-center border border-white rounded-3xl hover:rounded-md transition-all duration-300 w-6 h-6 cursor-pointer peer'>
					?
				</p>
				<div className='absolute scale-0 peer-hover:scale-100 origin-top-left transition top-4 -right-4 translate-x-full p-2 border border-white rounded-sm'>
					<p>M = 1000</p>
					<p>D = 500</p>
					<p>C = 100</p>
					<p>L = 50</p>
			...
			<p>
				Resultado: <span className={typeof result == 'string' ? 'font-serif' : ''}>{result}</span>
			</p>
		</div>
	);
}
